...
class BinaryMetricsViewSet(ViewSet):

    def create(self, request):
        """
        Store new binary metrics result in the database
        """
        ...
        api_serializer = BinaryMetricsSerializer(data=data)
        if not api_serializer.is_valid():
            return Response(api_serializer.errors, status=HTTP_400_BAD_REQUEST)

        api_serializer.save()
        db_serializer = BinaryMetricsResultSerializer(data=api_serializer.instance.convert_to_db_json())
